...
class RunningStack:
    ...
    def start(self) -> None:
        ...
        config = WebConfig(
            ...
            # Uploaded datasets are owned by a data node (which has the ndvi
            # skill), so the coordination flow can execute on them.
            upload_node_url=f"http://127.0.0.1:{self.data_b_port}",
        )
        ...
        def _run_coordinate(body: CoordinateRequest) -> dict[str, Any]:
            ...
            for r in data_results:
                ...
                if card.get("type") != "data":
                    continue
                # Change detection needs data with a temporal window that
                # overlaps the request; cards without temporal coverage (e.g.
                # freshly uploaded datasets) are listed but not planned.
                temporal = card.get("temporal") or {}
                has_temporal = bool(temporal.get("start") or temporal.get("end"))
                if body.capability == "change-detection" and not has_temporal:
                    skipped_no_temporal.append(card.get("id"))
                    continue
            ...
            report["steps"].append(
                {
                    ...
                    "skipped_no_temporal": skipped_no_temporal,

...
function createGovernance(ctx) {
  ...
  db.exec(`
    ...
    -- 案例的真实几何（GeoJSON）：栅格矢量化后的水面变化面、点位等。
    -- 存文件路径 + 属性，几何本身走受控读取接口发给前端。
    CREATE TABLE IF NOT EXISTS case_geometry (
      id TEXT PRIMARY KEY,
      caseId TEXT NOT NULL,
      name TEXT NOT NULL,
      kind TEXT NOT NULL DEFAULT 'geojson',
      path TEXT NOT NULL,
      properties TEXT NOT NULL DEFAULT '{}',
      createdAt TEXT NOT NULL,
      UNIQUE(caseId, name)
    );
    CREATE INDEX IF NOT EXISTS idx_case_geometry_case ON case_geometry(caseId);
  ...
  // 资产覆盖范围（JSON 数组）。矢量化的几何要复用它的可见性判定，所以这里必须有值。
  try { db.exec("ALTER TABLE asset_cards ADD COLUMN bbox TEXT"); } catch { /* 列已存在 */ }
  ...
  // 案例的**公开事实块**（国别汇总 Table 4、口径差异的合计数字、数据来源…）。
  // 平台只转发，不解释：这些数字是被研究案例自己带进来的，不是平台算出来的。
  try { db.exec('ALTER TABLE cases ADD COLUMN facts TEXT'); } catch { /* 列已存在 */ }
  ...
  const caseRow = (r) => ({
    ...
    // 案例自带的公开事实块（国别汇总 / 口径合计 / 数据来源）：随案例一起返回，
    // 案例页的"研究区域"与图例脚注都读它，不另开一次请求。
    facts: p(r.facts, null),
  ...
  function upsertCase(body, user, existingId = null) {
    ...
    if (existingId) {
      ...
      // 更新允许**局部**提交：只改一个字段（例如"清空范围"）不该被迫带上整份案例。
      // 标题仍旧不能变成空 —— 但"没提"与"改成空"是两件事。
      const title = 'title' in body ? body.title : prev.title;
      if (!title) throw Object.assign(new Error('title 不能为空'), { status: 422 });
      db.prepare(`UPDATE cases SET title=?, question=?, aoi=?, data=?, processing=?, outputs=?,
                  ...
                  updatedAt=? WHERE id=?`)
        .run(title, body.question ?? prev.question, body.aoi ?? prev.aoi,
          j(body.data ?? p(prev.data, [])), body.processing ?? prev.processing,
          j(body.outputs ?? p(prev.outputs, [])), body.provenance ?? prev.provenance,
          // 可空字段用 "键是否存在" 判断，而不是 ??：否则 **永远无法清空** 一个错的
          // bbox / 时间窗（显式传 null 会被当成"没改"）。编辑器"删掉范围"是真需求。
          j('bbox' in body ? body.bbox : p(prev.bbox, null)),
          j('temporal' in body ? body.temporal : p(prev.temporal, null)),
          'sensitivity' in body ? body.sensitivity : prev.sensitivity,
   ...
   */
  async function caseLayers(caseRowIn, { catalogue = null } = {}) {
    ...
    // 组成项（含 bbox）：血缘弧线的两端是"案例范围 ↔ 提供数据/算子的资产"。
    // 解析不到就是解析不到（可见性判定已经处理过"整案是否可见"），这里只如实报告。
    const cards = catalogue === null ? await loadCatalogue() : catalogue;
    const runtime = { catalogue: cards };
    const components = caseComponents(caseRowIn.id).map((c) => {
      const local = db.prepare('SELECT * FROM asset_cards WHERE id = ?').get(c.id);
      const card = cards.find((x) => x.id === c.id) || local;
      // 目录里的卡片用 spatial.bbox；平台镜像表用 bbox 列（JSON 数组）
      const cardBbox = Array.isArray(card?.spatial?.bbox) ? card.spatial.bbox : p(card?.bbox, null);
      return {
        id: c.id, role: c.role, step: c.step,
        // 名称给界面用（"所用数据"表里光看 id 认不出是什么）
        title: card?.title || c.id,
        bbox: Array.isArray(cardBbox) && cardBbox.length === 4 ? cardBbox : null,
        visibility: componentVisibility(c.id, runtime)
      };
    });

    const geometry = db.prepare('SELECT * FROM case_geometry WHERE caseId = ? ORDER BY name').all(caseRowIn.id)
      .map((row) => ({
        name: row.name,
        kind: row.kind,
        // 前端不拼路径：由平台给出受控读取地址（可见性在同一处判定）
        url: `/api/cases/${caseRowIn.id}/geometry/${row.name}`,
        properties: p(row.properties, {})
      }));
    ...
    return {
      ...
      components,
      geometry,
      // 案例自带的公开事实块（国别汇总、两套保护口径的合计…）。没有就是 null ——
      // 界面据此决定"显示数字"还是"显示未声明"，不去猜。
      facts: p(caseRowIn.facts, null),
      // 处理过程要能被"照着读"：把方案的步骤（id/kind/uses/说明/依赖）一并给出，
      // 前端才能像参照站那样列出编号步骤，而不是只显示一条运行记录。
      recipe: binding
        ? {
            recipeId: binding.recipeId,
            params: binding.params,
            contract: binding.paramContract,
            status: binding.status,
            entrypoint: (binding.recipe || {}).entrypoint || null,
            steps: ((binding.recipe || {}).steps || []).map((step) => ({
              id: step.id, kind: step.kind, uses: step.uses,
              description: step.description || '', depends_on: step.depends_on || []
            }))
          }
        : null,
  ...
  return async function handle(req, res, url) {
    ...
    try {
      ...
      if (isApi && at(1) === 'cases' && seg.length === 4 && at(3) === 'layers' && req.method === 'GET') {
        const visible = await requireVisibleCase(req, res, at(2));
        if (!visible) return true;
        const view = await caseLayers(caseRow(visible.row));
      ...
      // 案例几何（GeoJSON）：可见性判定与图层视图同一处，路径走白名单
      if (isApi && at(1) === 'cases' && seg.length === 5 && at(3) === 'geometry' && req.method === 'GET') {
        const visible = await requireVisibleCase(req, res, at(2));
        if (!visible) return true;
        const row = db.prepare('SELECT * FROM case_geometry WHERE caseId = ? AND name = ?').get(at(2), at(4));
        if (!row) return send(res, 404, { error: '该案例没有这份几何' }), true;
        try {
          const file = artifactPathFor({ outputs: { [row.name]: row.path } }, row.name);
          const fsMod = require('fs');
          if (!fsMod.existsSync(file)) {
            return send(res, 404, { error: '几何文件不在磁盘上（可能已被清理）' }), true;
          }
          res.writeHead(200, {
          ...
          return send(res, err.status || 500, { error: err.message }), true;
        }
      }

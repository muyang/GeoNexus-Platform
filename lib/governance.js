...
function createGovernance(ctx) {
  ...
  function upsertCase(body, user, existingId = null) {
    ...
    if (existingId) {
      ...
      db.prepare(`UPDATE cases SET title=?, question=?, aoi=?, data=?, processing=?, outputs=?,
                  provenance=?, bbox=?, temporal=?, status=?, geoCardId=?, runSpec=?, updatedAt=? WHERE id=?`)
    ...
    db.prepare(`INSERT INTO cases (id, title, question, aoi, data, processing, outputs, provenance, bbox,
                temporal, status, ownerUserId, geoCardId, runSpec, createdAt, updatedAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(caseId, body.title, body.question || null, body.aoi || null, j(body.data || []),
        ...
        j(body.bbox || null), j(body.temporal || null), body.status || 'draft', user?.id || null,
  ...
  function bindRecipe({ caseId = null, recipeId, parentRecipeId = null, recipe = null, params = {}, status = 'pending' }, user) {
    ...
    db.prepare(`INSERT INTO case_recipes (id, caseId, recipeId, parentRecipeId, params, paramContract,
                deliverables, visibility, status, ownerUserId, createdAt, updatedAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(rowId, caseId, recipeId, parentRecipeId, j(params), j(contractOf(recipe)),
        j(deliverableSpecsOf(recipe)), 'public', status, user?.id || null, now, now);
  ...
  return async function handle(req, res, url) {
    ...
    try {
      ...
      if (isApi && at(1) === 'recipes' && seg.length === 2 && req.method === 'POST') {
        ...
        try {
          const submitted = await recipes.registerRecipe(body.recipe, {
            status: body.status || 'pending', by: user.email || user.id, replace: Boolean(body.replace)
          });
          const binding = body.caseId
            ? bindRecipe({ caseId: body.caseId, recipeId: submitted.id, recipe: submitted.entry?.recipe || body.recipe, params: body.params || {}, status: 'pending' }, user)
            : null;
          audit(user, 'recipe.submit', submitted.id, { caseId: body.caseId || null });
          return send(res, 201, { submitted, binding }), true;
        } catch (err) {
          audit(user, 'recipe.submit', body.recipe?.id || null, { error: err.message }, 'failed');
          return send(res, err.status && err.status < 500 ? err.status : 502,
            { error: `提交方案失败：${err.message}`, detail: err.detail || null }), true;
        }
      }
      ...
      if (isApi && at(1) === 'recipes' && seg.length === 3 && at(2) === 'fork' && req.method === 'POST') {
        ...
        try {
          ...
          return send(res, 201, { forked, binding, caseId }), true;
        } catch (err) {
      ...
      if (isApi && at(1) === 'recipes' && seg.length === 3 && at(2) === 'run' && req.method === 'POST') {
        ...
        try {
          ...
          return send(res, err.status || 502, {
            error: err.message, code: err.code || null, field: err.field || null, detail: err.detail || null
          }), true;
        }
      }
      ...
      if (isApi && at(1) === 'cases' && seg.length === 4 && at(3) === 'layers' && req.method === 'GET') {
        const row = db.prepare('SELECT * FROM cases WHERE id = ?').get(at(2));
        if (!row) return send(res, 404, { error: '案例不存在' }), true;
        return send(res, 200, { view: caseLayers(caseRow(row)) }), true;
      }

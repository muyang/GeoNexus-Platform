...
| **OGE** | **资源** | **独立系统**：有自己的前后端、自己的用户注册系统（OGE 账号域），对外提供数据中心 / 计算中心 / AI 中心（解译库·样本库）/ 知识中心 / 管理中心 / 国产化适配 | 平台**主要消费其后端能力**（数据 / 算子 / 计算）；它同时是**默认 GeoNode 解决方案**，但**不是唯一资源节点**——平台未来接入其他 GeoNode。**两个账号域不做联邦** |
...
graph TB
    ...
    subgraph Res["④ 资源与执行层（GeoNode 抽象 · 可扩展）"]
        OGE["OGE（独立系统 · 资源平台）<br/>自有前端：门户/控制台　自有账号：OGE 账号域（独立注册）<br/>后端能力（平台消费）：数据中心 · 计算中心 · AI中心 · 知识中心 · 管理中心 · 国产化适配<br/>接入方式：GeoNode 外壳 + OGE Adapter（凭证托管换 tk）"]
...
### 3.3 资源 ← 心脏（OGE ← SDK）：OGE 是独立系统，平台只消费其后端能力

**先说清楚 OGE 是什么**：它**不是**本平台的一个模块，而是一个**独立系统**——有自己的门户/控制台、
自己的用户注册与登录（我们称之为 **OGE 账号域**）和自己的后端能力。本平台**主要消费其后端能力**
（数据、算子、计算），**不嵌入它的前端、也不代理它的登录**。

在协议层面它仍以 **GeoNode 身份**接入（见 `OGE_as_GeoNode_Solution.md`）：**GeoNode 标准化外壳**套在
**OGE 原生内核**外面，两层之间用 Runtime Adapter 翻译。对接的是它的**后端接口**（OpenAPI / OGC API）。
...
| 问题 | 决策 | 理由 |
|---|---|---|
| 平台用户要不要在 OGE 注册？ | **不要**。平台用自己在 OGE 的**应用凭证**（`mogan_oge_credential`，加密存储）在运行时换取 `tk`，**代表用户调用** | 用户只需一套账号；否则会出现"同一个人两套账号、权限对不上" |
| 那"谁用了多少"记在哪？ | **记在平台**（`mogan_quota` + `mogan_audit`）。OGE 侧只看到"平台这个应用" | 平台是唯一的用户账本，用量与配额才能对得上 |
| 某单位需要在 OGE 侧独立开户？ | 由**平台管理员代理开户**，并在映射表里登记（`mogan_oge_account_map`） | 防"影子账号"：用户自行注册的账号不在平台视野内，既记不到账也管不住权限 |
| 能不能做账号联邦（OGE 账号直接登录本平台）？ | **不做**。两个账号域相互独立 | 联邦会把身份权威一分为二，与"Platform 是唯一身份权威"直接冲突 |
...
>
> **完整设计见 [`docs/user-management.md`](user-management.md)**（账号模型、三条准入路径、
> 令牌契约、角色矩阵、数据权限投影规则、配额、个人中心板块与接口、8 条可验收判据）。
> 本节只保留结论与边界。
...
| | 8.典型应用 | 同上 |
| **资源平台 / OGE**（向隆刚） | 1.首页（李卫欣） | ④ OGE 独立系统「后端能力」1.首页（资源规模） |
...
1. OGE 的「5.知识中心」是**节点本地**知识库与检索问答（随节点走），与 GeoKG（**全局**领域知识图谱，
   可独立 / 可外挂 / 第三方可接入）**不是同一份知识**，不要合二为一。
2. **OGE 有自己的账号域**：本平台用户无需在 OGE 注册，能力由平台代表用户调用并记账；
   两个账号域**不做联邦**，也不允许用户自行注册形成影子账号（见 3.3）。

---

## 八、知识产品如何表达（案例 = 可复用的方案）

一个复杂案例（多份数据 + 算子 + 模型 + 文档）要能被**表达、展示、复用**，靠的不是
"把文件都传上去"，而是把四件事分层放好。哪一层放错，复用就会退化成"让对方照着我
的截图重做一遍"。

| 层 | 承载什么 | 权威在谁 | 平台做什么 |
|---|---|---|---|
| ① 内容 / 资产层 | 数据、模型、算子的本体（文件、服务、接口） | 各自的提供方（节点/OGE） | 只登记地址与访问方式，不搬运数据 |
| ② 结构 / 关系层 | GeoKG：实体与关系（**派生**自内容，带血缘） | SDK / GeoKG | 检索与呈现，不做语义推断 |
| ③ 元数据契约层 | **GeoCard**：这是什么、能做什么、由哪些资产组成 | SDK（schema 即规范） | 提交、转发审核、镜像状态 |
| ④ 能力 / 执行契约层 | **Recipe**：怎么跑（步骤、参数、交付物） | SDK（`geo.plan` 物化） | 提交运行、登记运行记录与交付物 |

四层之间是 **derive / describe / operate / reference**，**不是**"某层是另一层的表示"：

- GeoKG 不是"知识库的另一种存法"，它是从内容**派生**出来的关系视图，必须能追回来源；
- GeoCard 不是"知识图谱的卡片版"，它是资产的**元数据契约**；
- GeoCard 与 Recipe **不是**"描述与实现"的关系：卡片说这是什么，方案说怎么跑。
  一个案例可以"有方案没卡片"（草稿）或"有卡片没方案"（只有元数据，不可执行）。

### 一次复用的完整链路（平台侧）

```
目录 GET /api/recipes
  └─▶ fork POST /api/recipes/fork        新地址 + 我的参数；源方案一字未改
        └─▶ 审批 recipe-publish            派生件默认待审（与 GeoCard 发布同一条链路）
              └─▶ 发布案例（draft → published）
                    └─▶ 运行 POST /api/recipes/run
                          ├─ 先 geo.plan 物化（只看不跑，参数契约由 SDK 判定）
                          ├─ 按任务图逐步提交执行，上游产物按 step:// 引用传给下游
                          └─ 登记交付物 deliverables（结论，不是每一步的副产品）
                                └─▶ 图层视图 /api/cases/:id/layers → 四级 LOD + 双时间轴
```

### 三个容易踩的判断（已经写进代码与测试）

1. **参数契约必须存在，而且只有一份。** 没有它，fork 出来的案例是"死的"：没人知道
   哪些字段可以改、改到什么范围、哪些是作者钉死的。它在 SDK 定义一次（`ParamSpec`），
   卡片与方案共用；平台只把 SDK 的报错翻译成 400 + 字段名，**不复制一份校验规则**
   （复制出来的第二份迟早和第一份不一致）。
2. **可见性取最严的那一项。** 案例标 public、其中一份数据是 sensitive，那整案就对外
   不存在（404，不是 403：403 等于承认"存在一个你看不到的案例"）。只有**承载内容**的
   组成项参与判定 —— 算子与算力是代码与资源，把它们算进来会让每个案例都不可见。
3. **交付物是结论，不是中间产物。** 方案 `outputs` 里声明的才叫交付物；产物落在哪是
   运行环境的实现细节，公开接口不暴露磁盘路径，下载走白名单受控读取。

...
与 docs/user-management.md 同源同义；视觉语言与 platform-architecture.png 保持一致。
...
def box(ax, x, y, w, h, title="", items=(), kind="center", *, title_size=11.5,
...
fig, ax = plt.subplots(figsize=(17.6, 11.5), dpi=170)
ax.set_xlim(0, 100)
ax.set_ylim(0, 100)
ax.axis("off")

ax.text(50, 99.4, "用户管理模块设计：账号 · 注册 · 权限 · 个人中心",
        ha="center", va="top", fontsize=19.5, fontweight="bold", color="#1F4E79")
ax.text(50, 96.3, "一个账号中心（RuoYi sys_user）· 三种准入路径 · 三段式权限 · 个人中心只聚合不建表",
        ha="center", va="top", fontsize=10.5, color="#555555")

# ── ① 准入：三条路径 ─────────────────────────────────────────────────────
ax.text(1.2, 94.6, "① 准入：三条路径", fontsize=11, fontweight="bold", color="#1F4E79")
box(ax, 1.5, 86.6, 27.0, 7.4, "A. 公众自助注册（门户 / 开放社区）",
    ["· 图形验证码 + 邮箱/手机验证码 + 频控",
     "· 落「公众用户」部门 → 角色 public_visitor",
     "· 注册 ≠ 授权：注册后只能看，要能做得走审批"],
    "entry", title_size=10.5, item_size=8.2)
box(ax, 29.8, 86.6, 27.0, 7.4, "B. 机构开户 / 批量导入（后台）",
    ["· 部门必选 + 岗位 + 初始角色 + 初始密码",
     "· 首登强制改密（pwd_update_date 置空）",
     "· Excel 批量导入，落对应单位部门树"],
    "entry", title_size=10.5, item_size=8.2)
box(ax, 58.1, 86.6, 27.9, 7.4, "C. 服务账号签发（开发管理）",
    ["· app_id / app_secret 哈希存储 + 白名单",
     "· 独立配额与有效期，不绑自然人",
     "· 不建 sys_user 账号（mogan_app 表）"],
    "entry", title_size=10.5, item_size=8.2)

# ── ② 账号中心 + 令牌服务 ────────────────────────────────────────────────
ax.text(1.2, 84.4, "② 账号中心（唯一身份权威）", fontsize=11, fontweight="bold", color="#B26A00")
box(ax, 1.5, 69.8, 62.0, 14.0, "RuoYi 账号中心 —— 一个账号中心，不做第二套用户表",
    ...
    "account", title_size=12, item_size=8.4, lw=2.2)

box(ax, 65.0, 69.8, 21.0, 14.0, "令牌服务",
    ["RS256 JWT 唯一由平台签发：",
     "sub / userId / deptId / deptPath",
     "/ tenant / accountType / roles",
     "/ scopes / jti / exp",
     "",
     ...
     "  公钥分发（缓存+轮换）"],
    "token", title_size=11.5, item_size=8.2, lw=2.0)

arrow(ax, (32.0, 86.6), (32.0, 84.1), "开户 / 注册", color="#E8A33D", lw=1.8, style="=", ldx=9.0)

# ── ③ 权限三段 ───────────────────────────────────────────────────────────
ax.text(1.2, 67.4, "③ 权限管理：三段式（每段回答不同问题：能不能点 / 能看哪些 / 能用多少）",
        fontsize=11, fontweight="bold", color="#1F4E79")
box(ax, 1.5, 50.4, 27.0, 15.5, "① 功能权限 —— 能不能点",
    ["定义：sys_menu.perms（按钮级）",
     ...
     "例：system:user:add",
     "     card:publish"],
    "perm1", title_size=11, item_size=8.3)

box(ax, 29.8, 50.4, 27.0, 15.5, "② 数据与资源权限 —— 能看哪些",
    ...
    "perm2", title_size=11, item_size=8.3, lw=2.2)

box(ax, 58.1, 50.4, 27.9, 15.5, "③ 配额与算力额度 —— 能用多少",
    ["定义：mogan_quota（部门总额 +",
     "      个人上限，两级取小）",
     "执行：SDK Scheduler 准入",
     "      + OGE 调用限额",
     ...
     "",
     "准入阶段拒绝（带理由），不是跑到一半才炸"],
    "perm3", title_size=11, item_size=8.3)

arrow(ax, (25.0, 69.8), (25.0, 66.4), "账号与角色 → 三段权限", color="#E8A33D", lw=1.8, style="=", ldx=18.0)

# ── ④ 个人中心 / 后台管理 / 资源服务器 ───────────────────────────────────
ax.text(1.2, 48.2, "④ 落地：个人中心（聚合不建表）· 后台管理（RuoYi 原生 + 扩展）· 四系统作为资源服务器",
        fontsize=11, fontweight="bold", color="#1F4E79")
box(ax, 1.5, 27.4, 30.0, 19.0, "个人中心（计划 2.5）聚合视图 · 不建副本",
    ["我的资料 / 安全设置（改密·绑定·",
     "  两步验证·登出其他设备）",
     "我的组织与角色（只读）",
     ...
     "写操作只有：改资料 / 偏好 / 密码 /",
     "绑定 / 登出 / 撤销申请"],
    "center", title_size=11, item_size=8.2)

box(ax, 33.0, 27.4, 30.0, 19.0, "后台管理（计划 2.6）",
    ["RuoYi 原生直接用：",
     "  用户 · 部门 · 岗位 · 角色 · 菜单",
     "  字典 · 参数 · 通知 · 日志",
     "",
     ...
     "边界：OGE「管理中心」只管资源与服务",
     "自身运维，不存用户表"],
    "admin", title_size=11, item_size=8.2)

box(ax, 64.5, 27.4, 21.5, 19.0, "四系统 = 资源服务器",
    ["Platform：本地验签 +",
     "  perms + 数据范围",
     "",
     "SDK / 执行面：服务 API Key",
     ...
     "  kg:write",
     "",
     "OGE-GeoNode：JWT / 服务令牌",
     "  + OPA 主权策略"],
    "server", title_size=11, item_size=8.2)

arrow(ax, (43.0, 50.4), (43.0, 47.0), "权限在三处执行", color="#2F8F4A", lw=1.8, ldx=13.0)
elbow(ax, [(86.0, 76.8), (93.5, 76.8), (93.5, 36.9), (86.2, 36.9)], color="#8E5BA6", lw=1.6)
ax.text(93.5, 79.4, "令牌下发", ha="center", va="bottom", fontsize=7.8, color="#8E5BA6", fontweight="bold")
ax.text(93.5, 57.0, "JWT / 服务令牌\n（各资源服务器\n就地验签）", ha="center", va="center",
        fontsize=7.8, color="#8E5BA6", linespacing=1.5)

# ── ⑤ 审计 ───────────────────────────────────────────────────────────────
box(ax, 1.5, 14.4, 84.5, 8.6, "⑤ 审计（两处都落，且可关联）",
    ["sys_logininfor 登录日志：谁 / 何时 / 何 IP / 成功或失败原因        "
     "sys_oper_log 操作日志：谁 / 哪个接口 / 参数摘要 / 结果",
     "mogan_audit 任务与执行审计：谁（_identity）/ 执行了什么 / 用了哪个节点 / 产物在哪",
     "关联键 request_id：平台请求 ←→ GeoMCP 调用 ←→ 节点执行，三段串起来可追溯"],
    "audit", title_size=11, item_size=8.4, lw=1.6)

# ── 红线说明 ─────────────────────────────────────────────────────────────
ax.text(1.5, 12.2, "四条红线（评审重点）", fontsize=11, fontweight="bold", color="#C00000")
box(ax, 1.5, 3.2, 84.5, 8.2, "",
    ["1. 注册 ≠ 授权：自助注册只拿到「能看」，要「能做」必须走审批（机构用户一律管理员开户）",
     ...
     "4. 越权返回 404 而非 403：不泄露「资产存在但无权」这四个字背后的信息"],
    "entry", item_size=8.4)
ax.text(1.5, 1.4, "注：账号类型只决定「怎么来的、能不能登录」，权限一律由角色决定 —— 不把权限判断散落到类型分支里。",
        fontsize=8.4, color="#666666", va="bottom")

x0 = 2.0
for style, color, label in [("=", "#E8A33D", "开户 / 注册"), ("-", "#2F8F4A", "权限执行点"),
                            ("=", "#8E5BA6", "JWT 校验（资源服务器）")]:
    ax.add_patch(FancyArrowPatch((x0, 0.9), (x0 + 3.0, 0.9), arrowstyle="-|>",
                                 mutation_scale=11, linewidth=1.6, color=color,
                                 linestyle={"=": (0, (5, 2))}.get(style, "-"), zorder=5))
    ax.text(x0 + 3.6, 0.9, label, fontsize=8.6, va="center", color="#333333")
    x0 += 20.0
